...
# Stored as a single-element list to allow mutation without `global`.
_llm_instance: list[Gemini | None] = [None]
...
def summarize(text: str, max_words: int = 150) -> str:
    """Summarize the given text to approximately the requested word count.

    Args:
        text: The source text to summarize.
        max_words: Target maximum word count for the summary.
...
def reset_llm() -> None:
    """Reset the module-level LLM singleton to None, forcing re-initialization on next use."""

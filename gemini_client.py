...
# Module-level singleton so callers can reuse without re-initializing.
# Stored in a dict to allow mutation without the global statement (avoids PLW0603).
_state: dict[str, Gemini | None] = {"llm_instance": None}
...
def initialize_gemini_llm(
    ...
    """Initialize and register a Gemini LLM instance as the active singleton.
    ...
    Returns:
        Gemini: The newly created and registered LLM instance.
    """
    llm = Gemini(
...
def get_llm() -> Gemini:
    ...
    """
    if _state["llm_instance"] is None:
        return initialize_gemini_llm()
    return _state["llm_instance"]
...
def summarize(text: str, max_words: int = 150) -> str:
    """Summarize text to approximately a given word count using Gemini.
    ...
    Returns:
        str: The condensed summary text.
...
def reset_llm() -> None:
    """Clear the cached LLM singleton so the next call re-initializes it."""
    _state["llm_instance"] = None

...
# Stored in a mutable container to avoid the `global` statement in setters.
_llm_state: dict[str, Gemini | None] = {"instance": None}


def _get_instance() -> Gemini | None:
    return _llm_state["instance"]


def _set_instance(llm: Gemini | None) -> None:
...
def get_llm() -> Gemini:
    ...
    """
    if _get_instance() is None:
        return initialize_gemini_llm()
    return _get_instance()  # type: ignore[return-value]
...
def summarize(text: str, max_words: int = 150) -> str:
    """Summarize the given text to approximately max_words words using Gemini.
    ...
    Returns:
        str: A concise summary preserving key facts.
...
def reset_llm() -> None:
    """Reset the cached LLM singleton so the next call to get_llm re-initializes it."""
    _set_instance(None)

"""RAG engine for building and querying a vector index from PDF documents."""
import hashlib
import os
...
class RAGEngine:
    """Retrieval-Augmented Generation engine that indexes a PDF and answers queries against it."""

    def __init__(self, pdf_url, storage_dir="./storage"):
        """Initialize the RAG engine by loading or building a vector index for the given PDF URL.

        Args:
            pdf_url: URL of the PDF document to index.
            storage_dir: Directory used to persist and load cached index data.
        """
    ...
    def query(self, question):
        """Query the RAG engine and return a generated answer for the given question.

        Args:
            question: The question to answer using the indexed document.

        Returns:
            str: The generated response from the query engine.
        """
        print(f"\nQuery: {question}")
